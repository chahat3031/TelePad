...
export const createPost = async (req, res) => {
  try {
    ...
    const post = await Post.find().sort({createdAt:-1}); //returns all the posts till now
...
export const getUserPosts = async (req, res) => {
  try {
    ...
    const post = await Post.find({ userId }).sort({ createdAt: -1 }); //returns all the posts till now
    console.log(post);
    res.status(200).json(post);
  } catch (err) {
    res.status(404).json({ message: err.message });
